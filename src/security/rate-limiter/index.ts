...
export class RateLimiter {
  ...
  private createSecureFingerprint(ip: string, userAgent: string, acceptLang: string): string {
    ...
    return crypto.createHash('sha256').update(fingerprint).digest('hex').substring(0, 16); // Suficiente para identificación única
  }
  ...
  public getRemainingBlockTime(clientId: string): number {
    ...
    const remaining = blockUntil - this.clock();
