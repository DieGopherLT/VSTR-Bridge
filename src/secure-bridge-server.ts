...
import { SecurityMiddleware, SecureFileManager, SecurityConfig } from './security';
...
export interface VSCodeAdapter {
  ...
  openTextDocument(path: string): PromiseLike<vscode.TextDocument>;
  showTextDocument(doc: vscode.TextDocument): void;
  executeCommand(command: string, ...args: any[]): void;
  setEnvironmentVariable(name: string, value: string): void;
}

export interface SecureBridgeServerDependencies {
...
export class SecureBridgeServer {
  ...
  constructor(
    ...
    dependencies?: Partial<SecureBridgeServerDependencies>
  ) {
    this.vsCodeAdapter = vsCodeAdapter;

    this.fileManager = dependencies?.fileManager ?? new SecureFileManager();
    this.fileManager.initialize();

    this.securityMiddleware =
      dependencies?.securityMiddleware ?? new SecurityMiddleware(config, this.fileManager.getBridgeDirectory());
  }
  ...
  async start() {
    try {
      ...
      this.server.listen(this.port, 'localhost', () => {
        this.vsCodeAdapter.showInformationMessage('Secure VSTR Bridge active and ready');
      ...
      this.vsCodeAdapter.showErrorMessage(`Failed to start secure bridge: ${error}`);
    }
  }
  ...
  private registerSecureBridge() {
    ...
    try {
      ...
      this.vsCodeAdapter.setEnvironmentVariable('VSTR', this.port.toString());
      this.vsCodeAdapter.setEnvironmentVariable('VSTR_TOKEN', bridgeInfo.auth_token);
    } catch (error) {
      this.vsCodeAdapter.showErrorMessage(`Failed to register secure bridge: ${error}`);
    }
  }
  ...
  private handlePing(res: http.ServerResponse) {
    const response = {
      status: 'ok',
      version: '0.1.2',
      workspace: this.vsCodeAdapter.getWorkspaceName() || 'Unknown',
      port: this.port,
      secure: true,
  ...
  private createTerminal(config: TaskConfig): vscode.Terminal {
    ...
    return this.vsCodeAdapter.createTerminal(terminalOptions);
  }
  ...
  private showSecurityNotification() {
    this.vsCodeAdapter
      .showInformationMessage('Secure VSTR Bridge active and ready', 'View Status', 'Security Settings')
  ...
  showStatus() {
    ...
    const message = `Secure VSTR Bridge Status:
        - Status: Active
        - Workspace: ${this.vsCodeAdapter.getWorkspaceName()}
    ...
    this.vsCodeAdapter
      .showInformationMessage(message, 'View Logs', 'Security Config')
      .then((selection: string | undefined) => {
  ...
  private openAuditLogs() {
    ...
    this.vsCodeAdapter.openTextDocument(auditLogger.getLogPath()).then(
      (doc: vscode.TextDocument) => {
        this.vsCodeAdapter.showTextDocument(doc);
      },
      () => {
        this.vsCodeAdapter.showErrorMessage('Failed to open audit logs');
      }
    );
  }

  private showSecuritySettings() {
    this.vsCodeAdapter.executeCommand('workbench.action.openSettings', 'vstrBridge.security');
  }
